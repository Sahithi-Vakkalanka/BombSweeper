...
def generate_bombs(rows: int, cols: int):
    ...
    for _ in range(20):  # Increasing the number of bombs for larger grid
        x = randint(0, rows - 1)
        y = randint(0, cols - 1)
        grid[x][y] = 1
    return grid
...
def handle_click(x, y):
    ...
    if not is_valid_move(x, y, rows, cols) or buttons[x][y]["text"] != " ":
        return
    winsound.PlaySound("mixkit-classic-click-1117.wav", winsound.SND_ASYNC)
    if grid[x][y] == 1:
        # Loading the bomb image
        bomb_image = Image.open("bomb.jpeg") 
        bomb_image = bomb_image.resize((80, 50))  # Can adjust the size as needed
        bomb_photo = ImageTk.PhotoImage(bomb_image)

        # Configure all buttons with the bomb image
        for i in range(rows):
            for j in range(cols):
                if grid[i][j] == 1:
                    buttons[i][j].config(image=bomb_photo, width=30, height=30)
                    buttons[i][j].image = bomb_photo  # Keeping a reference to avoid garbage collection
        ...
        return
    if player == 1:
        buttons[x][y].config(text="O", bg="blue")  # Displaying safe cell
    else :
...
def setup_welcome():
    ...
    background_image = Image.open("blast.jpeg")  
    background_image = background_image.resize((1500, 1500))  #Can adjust the size as needed
    bg_image = ImageTk.PhotoImage(background_image)

    background_label = tk.Label(welcome_frame, image=bg_image)
    background_label.image = bg_image  # Keeping a reference to avoid garbage collection
...
def setup_gui():
    ...
    for i in range(rows):
        for j in range(cols):
            buttons[i][j] = tk.Button(root, text=" ", width=2, height=1,
                                      command=lambda i=i, j=j: handle_click(i, j))
            buttons[i][j].grid(row=i, column=j, sticky="nsew")

    # Displaying player colors below the grid
...
if __name__ == "__main__":
    root = tk.Tk()
    root.geometry("750x800")  # Increasing height and width to accommodate new buttons and resizing
    setup_welcome()